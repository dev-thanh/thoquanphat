               	...
               	<form action="{{ route('pages.build.post') }}" method="POST">
					...
					<div class="nav-tabs-custom">
				        <ul class="nav nav-tabs">
				        	<li class="active">
				            	<a href="#content" data-toggle="tab" aria-expanded="true">Câu hỏi thường gặp</a>
				            </li>
				            <li class="">
				    	...
				    	<div class="tab-pane active" id="content">
							<div class="row">
								<div class="repeater" id="repeater">
									<div class="form-group">
										<h4 class="text-center">Nội dung câu hỏi và trả lời</h4>
									</div>
										...
										<tbody id="sortable">
											@if (!empty($content))
												@foreach ($content as $key => $value)
													...
													@include('backend.repeater.row-faq')
												@endforeach
											@endif
										</tbody>
									</table>
									<div class="text-right">
										<button class="btn btn-primary" 
											onclick="repeater(event,this,'{{ route('get.layout') }}','.index', 'faq', '.faq')">Thêm
										</button>
									</div>
								</div>
							</div>
